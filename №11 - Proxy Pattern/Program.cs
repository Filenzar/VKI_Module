...
public class Proxy : ISubject
{
    ...
    public void Request(string request)
    {
        ...
        {
            Console.WriteLine("Срок действия кэша истек. Очистка кэша...");
            _cache.Clear();
            _lastCacheClearTime = DateTime.Now;
        }
        ...
        {
            Console.WriteLine($"Proxy: Возврат кэшированного результата по запросу: {request}");
            Console.WriteLine($"Кэшированный ответ: {_cache[request]}");
        }
        else
        {
            Console.WriteLine($"Proxy: Нет кеша для запроса. Пересылка в RealSubject.");
            _realSubject.Request(request);
            // Сохраняем результат в кэш (для примера просто сохраняем сам запрос как результат)
            _cache[request] = $"Обработан {request} в {DateTime.Now}";
        }
    }
